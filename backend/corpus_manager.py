...
class CorpusManager:
    ...
    def add_document(
        ...
        if self.retriever.index is not None:
            ...
            # 1. Prepare document (load → normalize → chunk)
            texts = self.retriever.prepare_document(pdf_path)
            ...
            # 4. Append vectors (memory only)
            self.retriever.append_vectors(embeddings, texts)
